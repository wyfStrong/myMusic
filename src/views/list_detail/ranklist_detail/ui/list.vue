...
<script>
// 网络请求
import { getRankListDetail, download_simulate } from '@/api/ranklist'
export default {
  data() {
    return {
      list: [],
  ...
  methods: {
    async onLoad() {
      ...
      let a = await getRankListDetail(id, this.i)
      // 翻页
      this.i++
      // 将请求数据写入数据源,深拷贝
      this.list = [...this.list, ...a.data.list]
      // 将请求的图片信息等数据存储另一个数据源, 传给父组件
      this.listdata = a.data
      this.$eventBus.$emit('getrankList', this.listdata)
